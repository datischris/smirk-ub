...
class BabyDataset(BaseDataset):
    ...
    def __getitem_aux__(self, index):
        ...
        image_filepath = os.path.join(self.config.dataset.Baby_path, files_list[k])
        landmarks_fan_filepath = os.path.join(self.config.dataset.Baby_fan_landmarks_path, files_list[k].replace('.jpg','.npy'))
        landmarks_mediapipe_filepath = os.path.join(self.config.dataset.Baby_mediapipe_landmarks_path, files_list[k].replace('.jpg','.npy'))
...
def get_datasets_Baby(config=None):
    file = "datasets/identity_Baby.txt" # provided by Baby dataset
    with open(file) as f:
        lines = f.readlines()
    ...
    for subject, file in zip(subjects, files):
        ...
        if not os.path.exists(os.path.join(config.dataset.Baby_mediapipe_landmarks_path, file.replace('.jpg','.npy').replace(".png",".npy"))):
            continue

        train_dict[subject].append(file)
        num_files += 1

    return BabyDataset(train_dict, config)
